...
public class ClassPathMapperScanner extends ClassPathBeanDefinitionScanner {
  ...
  public Set<BeanDefinitionHolder> doScan(String... basePackages) {
    // 交给spring完成包扫描，生成所有的BD
    Set<BeanDefinitionHolder> beanDefinitions = super.doScan(basePackages); // 此处属于spring 参见spring翻译
    if (beanDefinitions.isEmpty()) {
      logger.warn("No MyBatis mapper was found in '" + Arrays.toString(basePackages) + "' package. Please check your configuration.");
    } else {
      // mybatis 对默认的BD做处理，从而使得mapper接口被替代成FactoryBean，并且设置必要的属性
  ...
  private void processBeanDefinitions(Set<BeanDefinitionHolder> beanDefinitions) {
    ...
    for (BeanDefinitionHolder holder : beanDefinitions) {
      ...
      if (!explicitFactoryUsed) {
        ...
        // 设置依赖注入的方式为AUTOWIRE_BY_TYPE
        /**
         * spring 注入的能力包括 构造注入 反射注入 set注入 【接口注入】
         * 注入的方式包含名称注入  类型注入 等
         */
